   ...
   for(int i = 0; i < g_cycleCount; i++)
     {
      ...
      string line = StringFormat(
         "%d\t%d\t%s\t%d\t%d\t%.5f\t%.5f\t%.5f\t%.5f\t%d\t%d\t%d\t%.5f\t%d\t%d\t%.5f\t%d\t%d\t%.5f\t%d\t%.5f\t%.5f\t%s",
         c.id, c.magic, c.symbol, (int)c.direction, (int)c.state,
         c.baseTP, c.defaultSpread, c.maxSpread, c.startMaxSpread,
         (c.useSpreadInTP ? 1 : 0), (c.useSwapInTP ? 1 : 0), (int)c.swapMode,
         c.swapPerLotPerNight, c.tripleSwapDay, (c.useROCFilter ? 1 : 0),
         c.rocThreshold, c.rocPeriod, c.rocTF, c.slipTolerance, c.maxDeviation,
         c.referencePrice, c.pushOffsetPips, SerializeLayers(c));
...
bool LoadCycles()
  {
   ...
   if(StringFind(magicHdr, "DCAPRO_V1") < 0) { FileClose(h); return(false); }
   ...
   while(!FileIsEnding(h))
     {
      ...
      g_cycles[idx].swapPerLotPerNight = StringToDouble(f[12]);
